...
class BuddyPress_Birthday_Members {
	...
	public function get_birthdays_permissions_check( $request ) {
		...
		if ( 'friends' === $scope && ! is_user_logged_in() ) {
			return new WP_Error(
				'rest_forbidden',
				__( 'You must be logged in to view friends birthdays.', 'buddypress-birthday' ),
				array( 'status' => 401 )
			);
		}
	...
	public function get_birthdays( $request ) {
		...
		if ( ! $field_id ) {
			return new WP_Error(
				'no_birthday_field',
				__( 'No birthday field configured. Please configure it in Settings > BP Birthday.', 'buddypress-birthday' ),
				array( 'status' => 400 )
			);
		}
